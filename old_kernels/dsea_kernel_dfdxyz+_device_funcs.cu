...
__global__ void dns_du0dxyz(int32_t i_worker, int32_t order_in, int32_t order_out, int32_t problemsize,
	...
	if (gidx<block_ncc) {
		...
		odrhou0dy[gidx] = dns_pDer1(rhou0_dy_ll, rhou0_dy_l, rhou0_dy_r, rhou0_dy_rr, DY);
		odu0dy[gidx] = dns_pDer1(u0_dy_ll, u0_dy_l, u0_dy_r, u0_dy_rr, DY);
		odu0d2y[gidx] = dns_pDer2(u0_dy_ll, u0_dy_l, u0_dy_c, u0_dy_r, u0_dy_rr, DY);
		...
		odrhou0dz[gidx] = dns_pDer1(rhou0_dz_ll, rhou0_dz_l, rhou0_dz_r, rhou0_dz_rr, DZ);
		odu0dz[gidx] = dns_pDer1(u0_dz_ll, u0_dz_l, u0_dz_r, u0_dz_rr, DZ);
		odu0d2z[gidx] = dns_pDer2(u0_dz_ll, u0_dz_l, u0_dz_c, u0_dz_r, u0_dz_rr, DZ);
		...
		odrhou0dx[gidx] = dns_pDer1(sy_bc_ll *  rhou0_dx_ll, sy_bc_l * rhou0_dx_l, sy_bc_r * rhou0_dx_r, sy_bc_rr * rhou0_dx_rr, DX);
		odu0dx[gidx] = dns_pDer1(sy_bc_ll *  u0_dx_ll, sy_bc_l * rhou0_dx_l, sy_bc_r * rhou0_dx_r, sy_bc_rr * rhou0_dx_rr, DX);
		odu0d2x[gidx] = dns_pDer2(sy_bc_ll * u0_dx_ll, sy_bc_l * u0_dx_l, u0_dx_c, sy_bc_r * u0_dx_r, sy_bc_rr * u0_dx_rr, DX);
		odrhou0u0dx[gidx] = dns_pDer1(sy_bc_ll * rhou0_dx_ll * sy_bc_ll * u0_dx_ll,
																 sy_bc_l * rhou0_dx_l * sy_bc_l * u0_dx_l, 
																 sy_bc_r * rhou0_dx_r * sy_bc_r * u0_dx_r, 
																 sy_bc_rr * rhou0_dx_rr * sy_bc_rr * u0_dx_rr, DX);
		...
		odu0dxdy[gidx] = dns_pDer1(sy_bc_ll *  du0dy_dx_ll, sy_bc_l * du0dy_dx_l, sy_bc_r * du0dy_dx_r, sy_bc_rr * du0dy_dx_rr, DX);
		odu0dxdz[gidx] = dns_pDer1(sy_bc_ll *  du0dz_dx_ll, sy_bc_l * du0dz_dx_l, sy_bc_r * du0dz_dx_r, sy_bc_rr * du0dz_dx_rr, DX);
	}
}
...
__global__ void dns_du1dxyz(int32_t i_worker, int32_t order_in, int32_t order_out, int32_t problemsize,
	...
	if (gidx<block_ncc) {
		...
		odu1d2y[gidx] = dns_pDer2(u1_dy_ll, u1_dy_l, u1_dy_c, u1_dy_r, u1_dy_rr, DY);
		odrhou1u1dy[gidx] = dns_pDer1(rhou1_dy_ll * u1_dy_ll, rhou1_dy_l * u1_dy_l, rhou1_dy_r * u1_dy_r, rhou1_dy_rr * u1_dy_rr, DY);
		...
		odu1d2z[gidx] = dns_pDer2(u1_dz_ll, u1_dz_l, u1_dz_c, u1_dz_r, u1_dz_rr, DZ);
		...
		odu1d2x[gidx] = dns_pDer2(u1_dx_ll, u1_dx_l, u1_dx_c, u1_dx_r, u1_dx_rr, DX);
		...
		odu1dxdy[gidx]= dns_pDer1(du1dx_dy_ll, du1dx_dy_l, du1dx_dy_r, du1dx_dy_rr, DY);
		...
		odu1dydz[gidx] = dns_pDer1(du1dz_dy_ll,  du1dz_dy_l, du1dz_dy_r, du1dz_dy_rr, DY);
	}
}
...
__global__ void dns_du2dxyz(int32_t i_worker, int32_t order_in, int32_t order_out, int32_t problemsize,
	...
	if (gidx<block_ncc) {
		...
		odu2d2y[gidx] = dns_pDer2(u2_dy_ll, u2_dy_l, u2_dy_c, u2_dy_r, u2_dy_rr, DY);
		...
		odu2d2z[gidx] = dns_pDer2(u2_dz_ll, u2_dz_l, u2_dz_c, u2_dz_r, u2_dz_rr, DZ);
		odrhou2u2dz[gidx] = dns_pDer1(rhou2_dz_ll * u2_dz_ll, rhou2_dz_l * u2_dz_l, rhou2_dz_r * u2_dz_r, rhou2_dz_rr * u2_dz_rr, DZ);
		...
		odu2d2x[gidx] = dns_pDer2(u2_dx_ll, u2_dx_l, u2_dx_c, u2_dx_r, u2_dx_rr, DX);
		...
		odu2dxdz[gidx] = dns_pDer1(du2dx_dz_ll, du2dx_dz_l, du2dx_dz_r, du2dx_dz_rr, DZ);
		...
		odu2dydz[gidx] = dns_pDer1(du2dz_dy_ll, du2dz_dy_l, du2dz_dy_r, du2dz_dy_rr, DY);
	}
}
...
__global__ void dns_drhoETpdxyz(int32_t i_worker, int32_t order_in, int32_t order_out,
	...
	if (gidx<block_ncc) {
		...
		odrhody[gidx] = dns_pDer1(rho_dy_ll, rho_dy_l, rho_dy_r, rho_dy_rr, DY);
		odpdy[gidx] = dns_pDer1(p_dy_ll, p_dy_l, p_dy_r, p_dy_rr, DY);
		odpu1dy[gidx] = dns_pDer1(p_dy_ll * rhou1_dy_ll / rho_dy_ll, p_dy_l * rhou1_dy_l / rho_dy_l, p_dy_r * rhou1_dy_r / rho_dy_r, p_dy_rr * rhou1_dy_rr / rho_dy_rr, DY);
		odrhoEdy[gidx] = dns_pDer1(rhoE_dy_ll, rhoE_dy_l, rhoE_dy_r, rhoE_dy_rr, DY);
		odrhoEu1dy[gidx] = dns_pDer1(rhoE_dy_ll * rhou1_dy_ll / rho_dy_ll, rhoE_dy_l * rhou1_dy_l / rho_dy_l, rhoE_dy_r * rhou1_dy_r / rho_dy_r, rhoE_dy_rr * rhou1_dy_rr / rho_dy_rr, DY);
		odTd2y[gidx] = dns_pDer2(calT(p_dy_ll, rho_dy_ll), calT(p_dy_l, rho_dy_l), calT(p_dy_c, rho_dy_c), calT(p_dy_r, rho_dy_r), calT(p_dy_rr, rho_dy_rr), DY);
		...
		odrhodz[gidx] = dns_pDer1(rho_dz_ll, rho_dz_l, rho_dz_r, rho_dz_rr, DZ);
		odpdz[gidx] = dns_pDer1(p_dz_ll, p_dz_l, p_dz_r, p_dz_rr, DZ);
		odpu2dz[gidx] = dns_pDer1(p_dz_ll * rhou2_dz_ll / rho_dz_ll, p_dz_l * rhou2_dz_l / rho_dz_l, p_dz_r * rhou2_dz_r / rho_dz_r, p_dz_rr * rhou2_dz_rr / rho_dz_rr, DZ);
		odrhoEdz[gidx] = dns_pDer1(rhoE_dz_ll, rhoE_dz_l, rhoE_dz_r, rhoE_dz_rr, DZ);
		odrhoEu2dz[gidx] =  dns_pDer1(rhoE_dz_ll * rhou2_dz_ll / rho_dz_ll, rhoE_dz_l * rhou2_dz_l / rho_dz_l, rhoE_dz_r * rhou2_dz_r / rho_dz_r, rhoE_dz_rr * rhou2_dz_rr / rho_dz_rr, DZ);
		odTd2z[gidx] = dns_pDer2(calT(p_dz_ll, rho_dz_ll), calT(p_dz_l, rho_dz_l), calT(p_dz_c, rho_dz_c), calT(p_dz_r, rho_dz_r), calT(p_dz_rr, rho_dz_rr), DZ);	
		...
		odrhodx[gidx] = dns_pDer1(rho_dx_ll, rho_dx_l, rho_dx_r, rho_dx_rr, DX);
		odpdx[gidx] = dns_pDer1(p_dx_ll, p_dx_l, p_dx_r, p_dx_rr, DX);
		odpu0dx[gidx] = dns_pDer1(p_dx_ll * sy_bc_ll * rhou0_dx_ll / rho_dx_ll, 
															p_dx_l * sy_bc_l * rhou0_dx_l / rho_dx_l, 
															p_dx_r * sy_bc_r * rhou0_dx_r / rho_dx_r, 
															p_dx_rr * sy_bc_rr * rhou0_dx_rr / rho_dx_rr, DX);
		odrhoEdx[gidx] = dns_pDer1(rhoE_dx_ll, rhoE_dx_l, rhoE_dx_r, rhoE_dx_rr, DX);
		odrhoEu0dx[gidx] =  dns_pDer1(rhoE_dx_ll * sy_bc_ll * rhou0_dx_ll / rho_dx_ll, 
																	rhoE_dx_l * sy_bc_l * rhou0_dx_l / rho_dx_l, 
																	rhoE_dx_r * sy_bc_r * rhou0_dx_r / rho_dx_r, 
																	rhoE_dx_rr * sy_bc_rr * rhou0_dx_rr / rho_dx_rr, DX);
		odTd2z[gidx] = dns_pDer2(calT(p_dx_ll, rho_dx_ll), calT(p_dx_l, rho_dx_l), calT(p_dx_c, rho_dx_c), calT(p_dx_r, rho_dx_r), calT(p_dx_rr, rho_dx_rr), DX);	
	}

}

__global__ void dns_drhouiujdxyz(int32_t i_worker, int32_t order_in, int32_t order_out,
						/*order 0*/ double * __restrict__ irho_c,
						/*order 1*/ double * __restrict__ irho_l, double * __restrict__ irho_r,
						/*order 2*/ double * __restrict__ irho_ll, double * __restrict__ irho_rr,
						/*order 0*/ double * __restrict__ irhou0_c,
						...
						/*order 2*/ double * __restrict__ irhou2_ll, double * __restrict__ irhou2_rr,
						int sy_bc_ll, int sy_bc_l, int sy_bc_r, int sy_bc_rr,
						/*order 2*/ double * __restrict__ odrhou0u2dz,
						/*order 2*/ double * __restrict__ odrhou1u2dz,
						/*order 2*/ double * __restrict__ odrhou0u1dy,
						/*order 2*/ double * __restrict__ odrhou2u1dy,
						/*order 2*/ double * __restrict__ odrhou1u0dx,
						/*order 2*/ double * __restrict__ odrhou2u0dx) {
	// Thread Idx
	int32_t tidx = blockIdx.x*blockDim.x+threadIdx.x;
	// Global Idx
	int32_t gidx;
	...
	if (gidx<block_ncc) {
		...
		double rho_dz_ll, rho_dz_l, rho_dz_r, rho_dz_rr;
		rho_dz_ll = irho_c[dz_ll];
		rho_dz_l = irho_c[dz_l];
		rho_dz_r = irho_c[dz_r];
		rho_dz_rr = irho_c[dz_rr];

		double rhou2_dz_ll, rhou2_dz_l, rhou2_dz_r, rhou2_dz_rr;
		rhou2_dz_ll = irhou2_c[dz_ll];
		rhou2_dz_l = irhou2_c[dz_l];
		rhou2_dz_r = irhou2_c[dz_r];
		rhou2_dz_rr = irhou2_c[dz_rr];

		double u2_dz_ll, u2_dz_l, u2_dz_r, u2_dz_rr;
		u2_dz_ll = rhou2_dz_ll / rho_dz_ll;
		u2_dz_l = rhou2_dz_l / rho_dz_l;
		u2_dz_r = rhou2_dz_r / rho_dz_r;
		u2_dz_rr = rhou2_dz_rr / rho_dz_rr;

		double rhou0_dz_ll, rhou0_dz_l, rhou0_dz_r, rhou0_dz_rr;
		rhou0_dz_ll = irhou0_c[dz_ll];
		rhou0_dz_l = irhou0_c[dz_l];
		rhou0_dz_r = irhou0_c[dz_r];
		rhou0_dz_rr = irhou0_c[dz_rr];

		odrhou0u2dz[gidx] = dns_pDer1(rhou0_dz_ll * u2_dz_ll,  rhou0_dz_l * u2_dz_l, rhou0_dz_r * u2_dz_r, rhou0_dz_rr * u2_dz_rr, DZ);

		double rhou1_dz_ll, rhou1_dz_l, rhou1_dz_r, rhou1_dz_rr;
		rhou1_dz_ll = irhou1_c[dz_ll];
		rhou1_dz_l = irhou1_c[dz_l];
		rhou1_dz_r = irhou1_c[dz_r];
		rhou1_dz_rr = irhou1_c[dz_rr];

		odrhou1u2dz[gidx] = dns_pDer1(rhou1_dz_ll * u2_dz_ll,  rhou1_dz_l * u2_dz_l, rhou1_dz_r * u2_dz_r, rhou1_dz_rr * u2_dz_rr, DZ);


		double rho_dy_ll, rho_dy_l, rho_dy_r, rho_dy_rr;
		rho_dy_ll = irho_c[dy_ll];
		rho_dy_l = irho_c[dy_l];
		rho_dy_r = irho_c[dy_r];
		rho_dy_rr = irho_c[dy_rr];

		double rhou1_dy_ll, rhou1_dy_l, rhou1_dy_r, rhou1_dy_rr;
		rhou1_dy_ll = irhou1_c[dy_ll];
		rhou1_dy_l = irhou1_c[dy_l];
		rhou1_dy_r = irhou1_c[dy_r];
		rhou1_dy_rr = irhou1_c[dy_rr];

		double u1_dy_ll, u1_dy_l, u1_dy_r, u1_dy_rr;
		u1_dy_ll = rhou1_dy_ll / rho_dy_ll;
		u1_dy_l = rhou1_dy_l / rho_dy_l;
		u1_dy_r = rhou1_dy_r / rho_dy_r;
		u1_dy_rr = rhou1_dy_rr / rho_dy_rr;

		double rhou0_dy_ll, rhou0_dy_l, rhou0_dy_r, rhou0_dy_rr;
		rhou0_dy_ll = irhou0_c[dy_ll];
		rhou0_dy_l = irhou0_c[dy_l];
		rhou0_dy_r = irhou0_c[dy_r];
		rhou0_dy_rr = irhou0_c[dy_rr];

		odrhou0u1dy[gidx] = dns_pDer1(rhou0_dy_ll * u1_dy_ll,  rhou0_dy_l * u1_dy_l, rhou0_dy_r * u1_dy_r, rhou0_dy_rr * u1_dy_rr, DY);

		double rhou2_dy_ll, rhou2_dy_l, rhou2_dy_r, rhou2_dy_rr;
		rhou2_dy_ll = irhou2_c[dy_ll];
		rhou2_dy_l = irhou2_c[dy_l];
		rhou2_dy_r = irhou2_c[dy_r];
		rhou2_dy_rr = irhou2_c[dy_rr];

		odrhou2u1dy[gidx] = dns_pDer1(rhou2_dy_ll * u1_dy_ll,  rhou2_dy_l * u1_dy_l, rhou2_dy_r * u1_dy_r, rhou2_dy_rr * u1_dy_rr, DY);


		double rho_dx_ll, rho_dx_l, rho_dx_r, rho_dx_rr;
		rho_dx_ll = irho_ll[gidx];
		rho_dx_l = irho_l[gidx];
		rho_dx_r = irho_r[gidx];
		rho_dx_rr = irho_rr[gidx];

		double rhou0_dx_ll, rhou0_dx_l, rhou0_dx_r, rhou0_dx_rr;
		rhou0_dx_ll = irhou0_ll[gidx];
		rhou0_dx_l = irhou0_l[gidx];
		rhou0_dx_r = irhou0_r[gidx];
		rhou0_dx_rr = irhou0_rr[gidx];

		double u0_dx_ll, u0_dx_l, u0_dx_r, u0_dx_rr;
		u0_dx_ll = rhou0_dx_ll / rho_dx_ll;
		u0_dx_l = rhou0_dx_l / rho_dx_l;
		u0_dx_r = rhou0_dx_r / rho_dx_r;
		u0_dx_rr = rhou0_dx_rr / rho_dx_rr;

		double rhou1_dx_ll, rhou1_dx_l, rhou1_dx_r, rhou1_dx_rr;
		rhou1_dx_ll = irhou1_ll[gidx];
		rhou1_dx_l = irhou1_l[gidx];
		rhou1_dx_r = irhou1_r[gidx];
		rhou1_dx_rr = irhou1_rr[gidx];

		odrhou1u0dx[gidx] = dns_pDer1(rhou1_dx_ll * sy_bc_ll * u0_dx_ll, rhou1_dx_l * sy_bc_l * u0_dx_l, rhou1_dx_r * sy_bc_r * u0_dx_r, rhou1_dx_rr * sy_bc_rr * u0_dx_rr, DX);

		double rhou2_dx_ll, rhou2_dx_l, rhou2_dx_r, rhou2_dx_rr;
		rhou2_dx_ll = irhou2_ll[gidx];
		rhou2_dx_l = irhou2_l[gidx];
		rhou2_dx_r = irhou2_r[gidx];
		rhou2_dx_rr = irhou2_rr[gidx];

		odrhou2u0dx[gidx] = dns_pDer1(rhou2_dx_ll * sy_bc_ll * u0_dx_ll, rhou2_dx_l * sy_bc_l * u0_dx_l, rhou2_dx_r * sy_bc_r * u0_dx_r, rhou2_dx_rr * sy_bc_rr * u0_dx_rr, DX);	
...
void DS::caller_worker (double ** p_in, double ** p_out, int32_t i_part, int32_t i_super_cycle,
	...
	dns_drhouiujdxyz  <<<gridSize,threads_per_block,0,*stream>>> (0, 0, 0,
						&p_c[offset_rho], &p_l[offset_rho], &p_r[offset_rho], &p_ll[offset_rho], &p_rr[offset_rho],
						&p_c[offset_rhou0], &p_l[offset_rhou0], &p_r[offset_rhou0], &p_ll[offset_rhou0], &p_rr[offset_rhou0],
						&p_c[offset_rhou1], &p_l[offset_rhou1], &p_r[offset_rhou1], &p_ll[offset_rhou1], &p_rr[offset_rhou1],
						&p_c[offset_rhou2], &p_l[offset_rhou2], &p_r[offset_rhou2], &p_ll[offset_rhou2], &p_rr[offset_rhou2],
						sy_bc_ll, sy_bc_l, sy_bc_r, sy_bc_rr,
						(double*) d_drhou0u2dz, (double*) d_drhou1u2dz,
						(double*) d_drhou0u1dy, (double*) d_drhou2u1dy,
						(double*) d_drhou1u0dx, (double*) d_drhou2u0dx);
	...
	dns_Res_StageAdvance <<<gridSize_dns_Res,threads_per_block_dns_Res,0,*stream>>>(0, 0, 0,
