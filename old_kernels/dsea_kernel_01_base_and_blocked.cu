...
__global__ void dns_blocked(int32_t i_worker, int32_t order_in, int32_t order_out) {
	...
	if (tidx < 256) {
		printf("tidx: %d, gidx: %d, col_in_block, row_in_block, %d, %d\n", tidx, idx, col_in_block, row_in_block);
	}
}
...
void DS::caller_worker (double ** p_in, double ** p_out, int32_t i_part, int32_t i_super_cycle,
	...
	if (i_super_cycle == 0 && i_part == 0 && myID == 0 ) {
		cout << "------------------ Blocked" << BLOCKSIZE_Z <<", " << BLOCKSIZE_Y << ", " << WARPSIZE_Z <<", " << WARPSIZE_Y << "------------------ " << endl;
		dns_blocked<<<gridSize,threads_per_block,0,*stream>>>(0, 0, 0);
