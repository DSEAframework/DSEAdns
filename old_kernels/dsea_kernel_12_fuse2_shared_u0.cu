...
__global__ void dns_du0dxyz(int32_t i_worker, int32_t order_in, int32_t order_out, int32_t problemsize,
	...
	__shared__ double s_u0_ll[BLOCKSIZE_Y+4][BLOCKSIZE_Z+4]; // 4-wide halo
	__shared__ double s_u0_l[BLOCKSIZE_Y+4][BLOCKSIZE_Z+4]; // 4-wide halo
	__shared__ double s_u0_r[BLOCKSIZE_Y+4][BLOCKSIZE_Z+4]; // 4-wide halo
	__shared__ double s_u0_rr[BLOCKSIZE_Y+4][BLOCKSIZE_Z+4]; // 4-wide halo
	...
	if (gidx<block_ncc) {
		...
		s_u0_ll[rb][cb] = irhou0_ll[gidx] / s_rho_ll[rb][cb];
		s_u0_l[rb][cb] = irhou0_l[gidx] / s_rho_l[rb][cb];
		s_u0_r[rb][cb] = irhou0_r[gidx] / s_rho_r[rb][cb];
		s_u0_rr[rb][cb] = irhou0_rr[gidx] / s_rho_rr[rb][cb];
		...
		if (rb < 4) {
			...
			s_rho_ll[rb-2][cb] = irho_ll[dy_ll];
			s_rho_l[rb-2][cb] = irho_l[dy_ll];
			s_rho_r[rb-2][cb] = irho_r[dy_ll];
			s_rho_rr[rb-2][cb] = irho_rr[dy_ll];

			s_u0_ll[rb-2][cb] = irhou0_ll[dy_ll] / s_rho_ll[rb-2][cb];
			s_u0_l[rb-2][cb] = irhou0_l[dy_ll] / s_rho_l[rb-2][cb];
			s_u0_r[rb-2][cb] = irhou0_r[dy_ll] / s_rho_r[rb-2][cb];
			s_u0_rr[rb-2][cb] = irhou0_rr[dy_ll] / s_rho_rr[rb-2][cb];
		}
		if (rb >= BLOCKSIZE_Y) {
			IDX((NY+Y+2)%NY, Z, dy_rr, NZ);
			...
			s_rho_ll[rb+2][cb] = irho_ll[dy_rr];
			s_rho_l[rb+2][cb] = irho_l[dy_rr];
			s_rho_r[rb+2][cb] = irho_r[dy_rr];
			s_rho_rr[rb+2][cb] = irho_rr[dy_rr];

			s_u0_ll[rb+2][cb] = irhou0_ll[dy_rr] / s_rho_ll[rb+2][cb];
			s_u0_l[rb+2][cb] = irhou0_l[dy_rr] / s_rho_l[rb+2][cb];
			s_u0_r[rb+2][cb] = irhou0_r[dy_rr] / s_rho_r[rb+2][cb];
			s_u0_rr[rb+2][cb] = irhou0_rr[dy_rr] / s_rho_rr[rb+2][cb];
		}
		...
		if (cb < 4) {
			...
			s_rho_ll[rb][cb-2] = irho_ll[dz_ll];
			s_rho_l[rb][cb-2] = irho_l[dz_ll];
			s_rho_r[rb][cb-2] = irho_r[dz_ll];
			s_rho_rr[rb][cb-2] = irho_rr[dz_ll];

			s_u0_ll[rb][cb-2] = irhou0_ll[dz_ll] / s_rho_ll[rb][cb-2];
			s_u0_l[rb][cb-2] = irhou0_l[dz_ll] / s_rho_l[rb][cb-2];
			s_u0_r[rb][cb-2] = irhou0_r[dz_ll] / s_rho_r[rb][cb-2];
			s_u0_rr[rb][cb-2] = irhou0_rr[dz_ll] / s_rho_rr[rb][cb-2];
		}
		if (cb >= BLOCKSIZE_Z) {
			IDX(Y, (NZ+Z+2)%NZ, dz_rr, NZ);
			...
			s_rho_ll[rb][cb+2] = irho_ll[dz_rr];
			s_rho_l[rb][cb+2] = irho_l[dz_rr];
			s_rho_r[rb][cb+2] = irho_r[dz_rr];
			s_rho_rr[rb][cb+2] = irho_rr[dz_rr];

			s_u0_ll[rb][cb+2] = irhou0_ll[dz_rr] / s_rho_ll[rb][cb+2];
			s_u0_l[rb][cb+2] = irhou0_l[dz_rr] / s_rho_l[rb][cb+2];
			s_u0_r[rb][cb+2] = irhou0_r[dz_rr] / s_rho_r[rb][cb+2];
			s_u0_rr[rb][cb+2] = irhou0_rr[dz_rr] / s_rho_rr[rb][cb+2];
		}
		...
		odrhou0dy[gidx] = dns_pDer1(s_rho_c[rb-2][cb] * s_u0_c[rb-2][cb], s_rho_c[rb-1][cb] * s_u0_c[rb-1][cb], s_rho_c[rb+1][cb] * s_u0_c[rb+1][cb], s_rho_c[rb+2][cb] * s_u0_c[rb+2][cb], DY);
		odu0dy[gidx] = dns_pDer1(s_u0_c[rb-2][cb],  s_u0_c[rb-1][cb], s_u0_c[rb+1][cb], s_u0_c[rb+2][cb], DY);
		tmp0 += dns_pDer2(s_u0_c[rb-2][cb],  s_u0_c[rb-1][cb], s_u0_c[rb][cb], s_u0_c[rb+1][cb], s_u0_c[rb+2][cb], DY);

		odrhou0dz[gidx] = dns_pDer1(s_rho_c[rb][cb-2] * s_u0_c[rb][cb-2], s_rho_c[rb][cb-1] * s_u0_c[rb][cb-1], s_rho_c[rb][cb+1] * s_u0_c[rb][cb+1], s_rho_c[rb][cb+2] * s_u0_c[rb][cb+2], DZ);
		odu0dz[gidx] = dns_pDer1(s_u0_c[rb][cb-2],  s_u0_c[rb][cb-1], s_u0_c[rb][cb+1], s_u0_c[rb][cb+2], DZ);
		tmp0 += dns_pDer2(s_u0_c[rb][cb-2],  s_u0_c[rb][cb-1], s_u0_c[rb][cb], s_u0_c[rb][cb+1], s_u0_c[rb][cb+2], DZ);

		odrhou0dx[gidx] = dns_pDer1(sy_bc_ll *  s_rho_ll[rb][cb] * s_u0_ll[rb][cb], sy_bc_l * s_rho_l[rb][cb] * s_u0_l[rb][cb], 
			sy_bc_r * s_rho_r[rb][cb] * s_u0_r[rb][cb], sy_bc_rr * s_rho_rr[rb][cb] * s_u0_rr[rb][cb], DX);
		...
		Res_rhou0[gidx] = -0.5 * dns_pDer1(sy_bc_ll * s_rho_ll[rb][cb] * s_u0_ll[rb][cb] * sy_bc_ll * s_u0_ll[rb][cb],
													sy_bc_l * s_rho_l[rb][cb] * s_u0_l[rb][cb] * sy_bc_l * s_u0_l[rb][cb], 
													sy_bc_r * s_rho_r[rb][cb] * s_u0_r[rb][cb] * sy_bc_r * s_u0_r[rb][cb], 
													sy_bc_rr * s_rho_rr[rb][cb] * s_u0_rr[rb][cb] * sy_bc_rr * s_u0_rr[rb][cb], DX);
