...
__global__ void dns_du0dxyz(int32_t i_worker, int32_t order_in, int32_t order_out, int32_t problemsize,
	...
	__shared__ double s_rhou0_0[BLOCKSIZE_Y+4][BLOCKSIZE_Z+4]; // 4-wide halo
	__shared__ double s_rho_0[BLOCKSIZE_Y+4][BLOCKSIZE_Z+4]; // 4-wide halo

	__shared__ double s_rhou0_1[BLOCKSIZE_Y+4][BLOCKSIZE_Z+4]; // 4-wide halo
	__shared__ double s_rho_1[BLOCKSIZE_Y+4][BLOCKSIZE_Z+4]; // 4-wide halo
	...
	if (gidx<block_ncc) {
		...
		s_rhou0_0[rb][cb] = irhou0_c[gidx];
		s_rho_0[rb][cb] = irho_c[gidx];
		...
		if (rb < 4) {
			...
			s_rhou0_0[rb-2][cb] = irhou0_c[dy_ll];
			s_rho_0[rb-2][cb] = irho_c[dy_ll];
		}
		if (rb >= BLOCKSIZE_Y) {
			IDX((NY+Y+2)%NY, Z, dy_rr, NZ);
			s_rhou0_0[rb+2][cb] = irhou0_c[dy_rr];
			s_rho_0[rb+2][cb] = irho_c[dy_rr];
		}

		// get halos
		if (cb < 4) {
			IDX(Y, (NZ+Z-2)%NZ, dz_ll, NZ);
			s_rhou0_0[rb][cb-2] = irhou0_c[dz_ll];
			s_rho_0[rb][cb-2] = irho_c[dz_ll];
		}
		if (cb >= BLOCKSIZE_Z) {
			IDX(Y, (NZ+Z+2)%NZ, dz_rr, NZ);
			s_rhou0_0[rb][cb+2] = irhou0_c[dz_rr];
			s_rho_0[rb][cb+2] = irho_c[dz_rr];
		}

		__syncthreads();

		u0_dx_c = s_rhou0_0[rb][cb] / s_rho_0[rb][cb];
		rhou0_dx_c = s_rhou0_0[rb][cb];

		double tmp0 = 0;

		double u0_dy_ll, u0_dy_l, u0_dy_c, u0_dy_r, u0_dy_rr;
		u0_dy_ll = s_rhou0_0[rb-2][cb] / s_rho_0[rb-2][cb];
		u0_dy_l = s_rhou0_0[rb-1][cb] / s_rho_0[rb-1][cb];
		u0_dy_c = s_rhou0_0[rb][cb] / s_rho_0[rb][cb];
		u0_dy_r = s_rhou0_0[rb+1][cb] / s_rho_0[rb+1][cb];
		u0_dy_rr = s_rhou0_0[rb+2][cb] / s_rho_0[rb+2][cb];

		odrhou0dy[gidx] = dns_pDer1(s_rhou0_0[rb-2][cb], s_rhou0_0[rb-1][cb], s_rhou0_0[rb+1][cb], s_rhou0_0[rb+2][cb], DY);
		...
		u0_dz_ll = s_rhou0_0[rb][cb-2] / s_rho_0[rb][cb-2];
		u0_dz_l = s_rhou0_0[rb][cb-1] / s_rho_0[rb][cb-1];
		u0_dz_c = s_rhou0_0[rb][cb] / s_rho_0[rb][cb];
		u0_dz_r = s_rhou0_0[rb][cb+1] / s_rho_0[rb][cb+1];
		u0_dz_rr = s_rhou0_0[rb][cb+2] / s_rho_0[rb][cb+2];

		odrhou0dz[gidx] = dns_pDer1(s_rhou0_0[rb][cb-2], s_rhou0_0[rb][cb-1], s_rhou0_0[rb][cb+1], s_rhou0_0[rb][cb+2], DZ);
		odu0dz[gidx] = dns_pDer1(u0_dz_ll, u0_dz_l, u0_dz_r, u0_dz_rr, DZ);
		tmp0 += dns_pDer2(u0_dz_ll, u0_dz_l, u0_dz_c, u0_dz_r, u0_dz_rr, DZ);

		s_rhou0_1[rb][cb] = irhou0_ll[gidx];
		s_rho_1[rb][cb] = irho_ll[gidx];

		// get halos
		if (rb < 4) {
			IDX((NY+Y-2)%NY, Z, dy_ll, NZ);
			s_rhou0_1[rb-2][cb] = irhou0_ll[dy_ll];
			s_rho_1[rb-2][cb] = irho_ll[dy_ll];
		}
		if (rb >= BLOCKSIZE_Y) {
			IDX((NY+Y+2)%NY, Z, dy_rr, NZ);
			s_rhou0_1[rb+2][cb] = irhou0_ll[dy_rr];
			s_rho_1[rb+2][cb] = irho_ll[dy_rr];
		}

		// get halos
		if (cb < 4) {
			IDX(Y, (NZ+Z-2)%NZ, dz_ll, NZ);
			s_rhou0_1[rb][cb-2] = irhou0_ll[dz_ll];
			s_rho_1[rb][cb-2] = irho_ll[dz_ll];
		}
		if (cb >= BLOCKSIZE_Z) {
			IDX(Y, (NZ+Z+2)%NZ, dz_rr, NZ);
			s_rhou0_1[rb][cb+2] = irhou0_ll[dz_rr];
			s_rho_1[rb][cb+2] = irho_ll[dz_rr];
		}

		__syncthreads();

		u0_dx_ll = s_rhou0_1[rb][cb] / s_rho_1[rb][cb];
		rhou0_dx_ll = s_rhou0_1[rb][cb];

		double u0_dy_ll_dx_ll = s_rhou0_1[rb-2][cb] / s_rho_1[rb-2][cb];
		double u0_dy_l_dx_ll = s_rhou0_1[rb-1][cb] / s_rho_1[rb-1][cb];
		double u0_dy_r_dx_ll = s_rhou0_1[rb+1][cb] / s_rho_1[rb+1][cb];
		double u0_dy_rr_dx_ll = s_rhou0_1[rb+2][cb] / s_rho_1[rb+2][cb];

		double du0dy_dx_ll = dns_pDer1(u0_dy_ll_dx_ll, u0_dy_l_dx_ll, u0_dy_r_dx_ll, u0_dy_rr_dx_ll, DY);

		double u0_dz_ll_dx_ll =s_rhou0_1[rb][cb-2] / s_rho_1[rb][cb-2];
		double u0_dz_l_dx_ll = s_rhou0_1[rb][cb-1] / s_rho_1[rb][cb-1];
		double u0_dz_r_dx_ll = s_rhou0_1[rb][cb+1] / s_rho_1[rb][cb+1];
		double u0_dz_rr_dx_ll = s_rhou0_1[rb][cb+2] / s_rho_1[rb][cb+2];

		double du0dz_dx_ll = dns_pDer1(u0_dz_ll_dx_ll, u0_dz_l_dx_ll, u0_dz_r_dx_ll, u0_dz_rr_dx_ll, DZ);


		s_rhou0_0[rb][cb] = irhou0_l[gidx];
		s_rho_0[rb][cb] = irho_l[gidx];

		// get halos
		if (rb < 4) {
			IDX((NY+Y-2)%NY, Z, dy_ll, NZ);
			s_rhou0_0[rb-2][cb] = irhou0_l[dy_ll];
			s_rho_0[rb-2][cb] = irho_l[dy_ll];
		}
		if (rb >= BLOCKSIZE_Y) {
			IDX((NY+Y+2)%NY, Z, dy_rr, NZ);
			s_rhou0_0[rb+2][cb] = irhou0_l[dy_rr];
			s_rho_0[rb+2][cb] = irho_l[dy_rr];
		}

		// get halos
		if (cb < 4) {
			IDX(Y, (NZ+Z-2)%NZ, dz_ll, NZ);
			s_rhou0_0[rb][cb-2] = irhou0_l[dz_ll];
			s_rho_0[rb][cb-2] = irho_l[dz_ll];
		}
		if (cb >= BLOCKSIZE_Z) {
			IDX(Y, (NZ+Z+2)%NZ, dz_rr, NZ);
			s_rhou0_0[rb][cb+2] = irhou0_l[dz_rr];
			s_rho_0[rb][cb+2] = irho_l[dz_rr];
		}

		__syncthreads();

		u0_dx_l =s_rhou0_0[rb][cb] / s_rho_0[rb][cb];
		rhou0_dx_l = s_rhou0_0[rb][cb];

		
		double u0_dy_ll_dx_l = s_rhou0_0[rb-2][cb] / s_rho_0[rb-2][cb];
		double u0_dy_l_dx_l = s_rhou0_0[rb-1][cb] / s_rho_0[rb-1][cb];
		double u0_dy_r_dx_l = s_rhou0_0[rb+1][cb] / s_rho_0[rb+1][cb];
		double u0_dy_rr_dx_l = s_rhou0_0[rb+2][cb] / s_rho_0[rb+2][cb];

		double du0dy_dx_l = dns_pDer1(u0_dy_ll_dx_l, u0_dy_l_dx_l, u0_dy_r_dx_l, u0_dy_rr_dx_l, DY);

		double u0_dz_ll_dx_l =s_rhou0_0[rb][cb-2] / s_rho_0[rb][cb-2];
		double u0_dz_l_dx_l = s_rhou0_0[rb][cb-1] / s_rho_0[rb][cb-1];
		double u0_dz_r_dx_l = s_rhou0_0[rb][cb+1] / s_rho_0[rb][cb+1];
		double u0_dz_rr_dx_l = s_rhou0_0[rb][cb+2] / s_rho_0[rb][cb+2];

		double du0dz_dx_l = dns_pDer1(u0_dz_ll_dx_l, u0_dz_l_dx_l, u0_dz_r_dx_l, u0_dz_rr_dx_l, DZ);


		s_rhou0_1[rb][cb] = irhou0_r[gidx];
		s_rho_1[rb][cb] = irho_r[gidx];

		// get halos
		if (rb < 4) {
			IDX((NY+Y-2)%NY, Z, dy_ll, NZ);
			s_rhou0_1[rb-2][cb] = irhou0_r[dy_ll];
			s_rho_1[rb-2][cb] = irho_r[dy_ll];
		}
		if (rb >= BLOCKSIZE_Y) {
			IDX((NY+Y+2)%NY, Z, dy_rr, NZ);
			s_rhou0_1[rb+2][cb] = irhou0_r[dy_rr];
			s_rho_1[rb+2][cb] = irho_r[dy_rr];
		}

		// get halos
		if (cb < 4) {
			IDX(Y, (NZ+Z-2)%NZ, dz_ll, NZ);
			s_rhou0_1[rb][cb-2] = irhou0_r[dz_ll];
			s_rho_1[rb][cb-2] = irho_r[dz_ll];
		}
		if (cb >= BLOCKSIZE_Z) {
			IDX(Y, (NZ+Z+2)%NZ, dz_rr, NZ);
			s_rhou0_1[rb][cb+2] = irhou0_r[dz_rr];
			s_rho_1[rb][cb+2] = irho_r[dz_rr];
		}

		__syncthreads();

		double u0_dy_ll_dx_r = s_rhou0_1[rb-2][cb] / s_rho_1[rb-2][cb];
		double u0_dy_l_dx_r = s_rhou0_1[rb-1][cb] / s_rho_1[rb-1][cb];
		double u0_dy_r_dx_r = s_rhou0_1[rb+1][cb] / s_rho_1[rb+1][cb];
		double u0_dy_rr_dx_r = s_rhou0_1[rb+2][cb] / s_rho_1[rb+2][cb];

		double du0dy_dx_r = dns_pDer1(u0_dy_ll_dx_r, u0_dy_l_dx_r, u0_dy_r_dx_r, u0_dy_rr_dx_r, DY);

		double u0_dz_ll_dx_r =s_rhou0_1[rb][cb-2] / s_rho_1[rb][cb-2];
		double u0_dz_l_dx_r = s_rhou0_1[rb][cb-1] / s_rho_1[rb][cb-1];
		double u0_dz_r_dx_r = s_rhou0_1[rb][cb+1] / s_rho_1[rb][cb+1];
		double u0_dz_rr_dx_r = s_rhou0_1[rb][cb+2] / s_rho_1[rb][cb+2];

		double du0dz_dx_r = dns_pDer1(u0_dz_ll_dx_r, u0_dz_l_dx_r, u0_dz_r_dx_r, u0_dz_rr_dx_r, DZ);


		s_rhou0_0[rb][cb] = irhou0_rr[gidx];
		s_rho_0[rb][cb] = irho_rr[gidx];

		// get halos
		if (rb < 4) {
			IDX((NY+Y-2)%NY, Z, dy_ll, NZ);
			s_rhou0_0[rb-2][cb] = irhou0_rr[dy_ll];
			s_rho_0[rb-2][cb] = irho_rr[dy_ll];
		}
		if (rb >= BLOCKSIZE_Y) {
			IDX((NY+Y+2)%NY, Z, dy_rr, NZ);
			s_rhou0_0[rb+2][cb] = irhou0_rr[dy_rr];
			s_rho_0[rb+2][cb] = irho_rr[dy_rr];
		}

		// get halos
		if (cb < 4) {
			IDX(Y, (NZ+Z-2)%NZ, dz_ll, NZ);
			s_rhou0_0[rb][cb-2] = irhou0_rr[dz_ll];
			s_rho_0[rb][cb-2] = irho_rr[dz_ll];
		}
		if (cb >= BLOCKSIZE_Z) {
			IDX(Y, (NZ+Z+2)%NZ, dz_rr, NZ);
			s_rhou0_0[rb][cb+2] = irhou0_rr[dz_rr];
			s_rho_0[rb][cb+2] = irho_rr[dz_rr];
		}

		__syncthreads();


		double u0_dy_ll_dx_rr = s_rhou0_0[rb-2][cb] / s_rho_0[rb-2][cb];
		double u0_dy_l_dx_rr = s_rhou0_0[rb-1][cb] / s_rho_0[rb-1][cb];
		double u0_dy_r_dx_rr = s_rhou0_0[rb+1][cb] / s_rho_0[rb+1][cb];
		double u0_dy_rr_dx_rr = s_rhou0_0[rb+2][cb] / s_rho_0[rb+2][cb];

		double du0dy_dx_rr = dns_pDer1(u0_dy_ll_dx_rr, u0_dy_l_dx_rr, u0_dy_r_dx_rr, u0_dy_rr_dx_rr, DY);

		double u0_dz_ll_dx_rr =s_rhou0_0[rb][cb-2] / s_rho_0[rb][cb-2];
		double u0_dz_l_dx_rr = s_rhou0_0[rb][cb-1] / s_rho_0[rb][cb-1];
		double u0_dz_r_dx_rr = s_rhou0_0[rb][cb+1] / s_rho_0[rb][cb+1];
		double u0_dz_rr_dx_rr = s_rhou0_0[rb][cb+2] / s_rho_0[rb][cb+2];
		...
		u0_dx_r = s_rhou0_1[rb][cb] / s_rho_1[rb][cb];
		rhou0_dx_r = s_rhou0_1[rb][cb];
		u0_dx_rr = s_rhou0_0[rb][cb] / s_rho_0[rb][cb];
		rhou0_dx_rr = s_rhou0_0[rb][cb];
