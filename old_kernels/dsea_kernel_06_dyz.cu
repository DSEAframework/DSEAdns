...
__global__ void dns_dfgdyz(int32_t i_worker, int32_t order_in, int32_t order_out,
	...
	if (gidx<block_ncc) {
		...
		dfgdy[gidx] = 1/DY * (1./12. * (f[dy_ll] * g[dy_ll]) - 2./3. * (f[dy_l] * g[dy_l]) 
								+ 2./3. * (f[dy_r] * g[dy_r]) - 1./12. * (f[dy_rr] * g[dy_rr])); 
		...
		dfgdz[gidx] = 1/DZ * (1./12. * (f[dz_ll] * g[dz_ll]) - 2./3. * (f[dz_l] * g[dz_l]) 
							+ 2./3. * (f[dz_r] * g[dz_r]) - 1./12. * (f[dz_rr] * g[dz_rr])); 
	}
}
...
__global__ void dns_dfd2yz(int32_t i_worker, int32_t order_in, int32_t order_out,
	...
	if (gidx<block_ncc) {
		...
		dfd2y[gidx] = 1/(DY*DY) * (-1./12. * f[dy_ll] + 4./3. * f[dy_l] - 5./2. * f[gidx] + 4./3. * f[dy_r] - 1./12. * f[dy_rr]); 
		...
		dfd2z[gidx] = 1/(DZ*DZ) * (-1./12. * f[dz_ll] + 4./3. * f[dz_l] - 5./2. * f[gidx] + 4./3. * f[dz_r] - 1./12. * f[dz_rr]); 
	}
}
